defmodule AshDurableReactor.Backends.Sqlite do
  @moduledoc false

  defmacro define_backend(opts) do
    quote do
      AshDurableReactor.Backends.Sqlite.__define_backend__(__ENV__, unquote(Macro.escape(opts)))
    end
  end
  ...
  def __define_backend__(env, opts) do
    ...
    unless Code.ensure_loaded?(AshSqlite.DataLayer) do
      raise "ash_sqlite is required to use `durable do; sqlite ... end`"
    end
  ...
  def define_backend_quoted(opts) do
    quote do
      AshDurableReactor.Backends.Sqlite.__define_backend__(
        __ENV__,
  ...
  defp modules(opts) do
    root = Module.concat([Keyword.fetch!(opts, :repo), AshDurableReactor, Sqlite])
